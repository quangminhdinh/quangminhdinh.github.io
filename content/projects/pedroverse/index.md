...
The UI is built on the **Blender Python API**; style transfer runs in TensorFlow, classical filters in OpenCV / scikit-image, and the Paint Transformer in PyTorch — each component invoked via subprocess to dodge Blender's module-import quirks, then packaged as a single add-on. Even the least-optimised method takes at most ~1 minute per asset.
